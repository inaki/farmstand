...
farmstand relies extensively on environment settings which **will not work with Apache/mod_wsgi setups**. It has been deployed successfully with both Gunicorn/Nginx and even uWSGI/Nginx.

For configuration purposes, the following table maps the 'farmstand' environment variables to their Django setting:
...
DJANGO_DEFAULT_FROM_EMAIL               DEFAULT_FROM_EMAIL          n/a                                            "farmstand <noreply@example.com>"
DJANGO_SERVER_EMAIL                     SERVER_EMAIL                n/a                                            "farmstand <noreply@example.com>"
DJANGO_EMAIL_SUBJECT_PREFIX             EMAIL_SUBJECT_PREFIX        n/a                                            "[farmstand] "
...
.. code-block:: bash
    ...
    cd farmstand
    celery -A farmstand.taskapp worker -l info
...
To run the tests, enter the farmstand/tests directory and run the following commands::
...
Postgres is saving its database files to `/data/farmstand/postgres` by default. Change that if you wan't
...
Place it in `/etc/supervisor/conf.d/farmstand.conf` and run::

    supervisorctl reread
    supervisorctl start farmstand
